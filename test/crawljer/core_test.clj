...
(def mock-path "/tmp/crawljer/example.com")

(defn setup-dirs
  []
  (fs/delete-dir "/tmp/crawljer/"))

(deftest files-test
  (testing "It should return the proper filename"
    (let [filenames ["first" "second.php"]
          urls ["http://first" "https://example.com/hello/world/10/12/second.php/"]]
      (is (= filenames (map get-filename-from-url urls)))))
  (testing "It should create a directory when not present"
    (setup-dirs)
    (let [path "/tmp/crawljer/"
          dir (create-directory "/tmp/crawljer/")]
      (is (fs/exists? dir))
      (setup-dirs)
      (is (not (fs/exists? dir)))))
  (testing "It should write file"
    (setup-dirs)
    (let [doc mock-page]
      (is (not (fs/exists? mock-path)))
      (fs/mkdirs (fs/parent mock-path))
      (write-doc doc)
      (is (fs/exists? mock-path)))))
...
(deftest add-urls-to-chan-test
  (testing "URLs should be added to channel"))
